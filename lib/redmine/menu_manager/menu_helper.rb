  ...
  def build_wiki_menus(project)
    ...
    WikiMenuItem.main_items(project_wiki).each do |main_item|
      Redmine::MenuManager.loose :project_menu do |menu|
        ...
        menu.push :"#{main_item.item_class}_new_page", {:action=>"new_child", :controller=>"/wiki", :id => CGI.escape(main_item.title) },
        ...
        menu.push :"#{main_item.item_class}_toc", {:action => 'index', :controller => '/wiki', :id => CGI.escape(main_item.title)}, :param => :project_id, :caption => :label_table_of_contents, :parent => "#{main_item.item_class}".to_sym if main_item.index_page
